...
public class PlaybackManager : MonoBehaviour {
   ...
   public void Play() {
      playing = true;
        ...
        //Station.instance.ScheduleNextDownBeatTime();

        if (Station.instance.nextDownBeatTime - AudioSettings.dspTime >= .4)
        {

            nextBeatTime = Station.instance.nextDownBeatTime - .15f; //0.15f is the Playback Buffer
        }
    }
   ...
   public void Rewind() {
    ...
    public void TempoSet(string input)
    {
        int bpmNew = int.Parse(input);
        ...
        DisplayManagerMix.Instance.SetTempo(bpmCurr);
        SaveState();
    }
